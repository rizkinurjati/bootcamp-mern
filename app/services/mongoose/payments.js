...
const getAllPayments = async (req) => {
  let condition = { organizer: req.user.organizer }; //tampilak berdasarkan organizer
...
const createPayments = async (req) => {
  ...
  await checkingImage(image); //check image
...
const getOnePayments = async (req) => {
  ...
  const result = await Payments.findOne({
    _id: id,
    organizer: req.user.organizer,
  }) // mengambil id payments dari parameter
    .populate({
      path: 'image',
      select: '_id name',
    })
...
const updatePayments = async (req) => {
  ...
  const check = await Payments.findOne({
    type,
    organizer: req.user.organizer,
    _id: { $ne: id }, // mengecek id payments di table lain
  });
